...
LIBSEL4_AUTO:= $(call select_from_ports,sel4)/src/kernel/sel4/configs/am335x
...
#
include/sel4/sel4_arch/%.h: $(LIBSEL4_DIR)/sel4_arch_include/aarch32/sel4/sel4_arch/%.h
	$(VERBOSE)mkdir -p $(dir $@)
	$(VERBOSE)ln -sf $< $@

include/sel4/arch/%.h: $(LIBSEL4_DIR)/arch_include/arm/sel4/arch/%.h
	$(VERBOSE)mkdir -p $(dir $@)
	$(VERBOSE)ln -sf $< $@
...
include/sel4/plat/api/%.h: $(LIBSEL4_DIR)/sel4_plat_include/am335x/sel4/plat/api/%.h
	$(VERBOSE)mkdir -p $(dir $@)
	$(VERBOSE)ln -sf $< $@
...
include/sel4/sel4_arch/types.pbf: $(LIBSEL4_DIR)/sel4_arch_include/aarch32/sel4/sel4_arch/types.bf include/sel4/autoconf.h
	$(MSG_CONVERT)$(notdir $@)
	$(VERBOSE)mkdir -p $(dir $@)
	$(VERBOSE)$(CPP) -Iinclude/sel4 -I$(LIBSEL4_DIR)/arch_include/arm -P $< >$@
...
include/sel4/sel4_arch/invocation.h: $(LIBSEL4_DIR)/sel4_arch_include/aarch32/interfaces/sel4arch.xml
	$(MSG_CONVERT)$(notdir $@)
	$(VERBOSE)mkdir -p $(dir $@)
	$(VERBOSE)python $(LIBSEL4_DIR)/tools/invocation_header_gen.py \
	                 --xml $< --libsel4 --sel4_arch --dest $@

include/sel4/arch/invocation.h: $(LIBSEL4_DIR)/arch_include/arm/interfaces/sel4arch.xml
	$(MSG_CONVERT)arch/$(notdir $@)
	$(VERBOSE)mkdir -p $(dir $@)
	$(VERBOSE)python $(LIBSEL4_DIR)/tools/invocation_header_gen.py \
	                 --xml $< --libsel4 --arch --dest $@

SEL4_CLIENT_H_SRC := $(LIBSEL4_DIR)/sel4_arch_include/aarch32/interfaces/sel4arch.xml \
                     $(LIBSEL4_DIR)/arch_include/arm/interfaces/sel4arch.xml \
...
include/interfaces/sel4_client.h: $(SEL4_CLIENT_H_SRC)
	...
	$(VERBOSE)python $(LIBSEL4_DIR)/tools/syscall_stub_gen.py \
	                 --buffer -a aarch32 --word-size 32 -o $@ $(SEL4_CLIENT_H_SRC)
