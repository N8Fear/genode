...
Am335x_driver& Am335x_driver::factory(Genode::Env &env)
{
	static Am335x_driver driver(env);
	return driver;
}
...
struct Main
{
	...
	:
		...
		driver(Am335x_driver::factory(env)),
		root(&env.ep().rpc_ep(), &sliced_heap, driver)
	{
		using namespace Genode;

		log("--- am335x gpio driver ---");
